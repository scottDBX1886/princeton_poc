...
**Run:**
```bash
databricks bundle run sftp_ingest -t dev --profile princeton_poc
```
...
**Run:**
```bash
databricks bundle run orchestration_demo -t dev --profile princeton_poc
```
...
```bash
databricks bundle summary -t dev --profile princeton_poc | grep -A2 e9_monitoring
```
...
databricks bundle validate -t dev  --profile princeton_poc   # SE-37: all three targets validate…
databricks bundle validate -t qa   --profile princeton_poc   #   …same code, three catalogs
databricks bundle validate -t prod --profile princeton_poc
