...
|----|----------|-----------|--------|--------|
| SE-01 | Relational DB ingestion — full extract | E2 | ✅ built & verified (Lakeflow Connect query-based ingestion, pre-built in workspace) | — n/a (managed connector walkthrough) |
| SE-02 | Relational DB ingestion — custom SQL (joins/agg/window/proc) | E2 | ✅ built & verified (Lakeflow Connect query-based + CDC connectors, pre-built in workspace) | — n/a (managed connector walkthrough) |
...
| DS-07 | Scheduling/operationalizing a notebook or script | DS-F | ✅ built & verified — **re-verified in the customer wksp 2026-08-26**: job TERMINATED SUCCESS, 5,000,000 enrollments / 30,000 students / 5,000 courses, avg GPA 3.0841, 249,652 withdrawals (all matching the retired-workspace baseline); idempotent across 2 run_dates; serverless no-`.cache()` fix intact | 🟢 tested (generated notebook + job YAML; table-collision gap fixed) |
...
| PA-09 | Row-level security — attribute-based filtering | PA-C | ✅ built & verified in customer wksp (2026-08-23: **enforcement proven** — as `account users`, 2,963 of 30,000 rows; departments [5,12,24] only) | 🟢 tested (customer wksp 2026-08-25, both halves: writes only to _prompt objects, own filter_by_department_prompt, session_user() branched restricted-FIRST, read real dept_ids from data, adopted the before/after shared-baseline guard) |
| PA-10 | Row-level security — dynamic policy by user identity | PA-C | ✅ built & verified in customer wksp (2026-08-23: department_access keyed on session_user() and seeded with the ROLE NAME; one INSERT widened access with no policy change) | 🟢 tested (customer wksp 2026-08-25: own department_access_prompt at depts 16/24/35; SHARED department_access untouched at 5/12/24 — 2,963 rows still reproduce) |
| PA-11 | Security policy testing & validation ("faux user") | PA-D | ✅ built & verified in customer wksp (2026-08-23: RBAC role switching IS the faux-user mechanism — both mechanisms run: parameterised twin for branch logic, real role switch for enforcement) | 🟢 tested (customer wksp 2026-08-25, both halves: harness dropped with no litter, labels branch-logic vs enforcement; restricted run confirmed via table_lineage — as `account users` it read column_masks/row_filters/routines) |
| PA-12 | Security policy audit & documentation | PA-D | ✅ built & verified in customer wksp (2026-08-23: information_schema.column_masks + row_filters — 4 masks, 2 filters, all in admin_demo, none on the shared foundation) | 🟢 tested (customer wksp 2026-08-25: inventory over column_masks + row_filters + routines, coverage-gap LEFT JOIN, privilege-escalation check on ALL_PRIVILEGES/CREATE_FUNCTION/MODIFY) |
...
| Admin (PA) | **10 — persona complete** (PA-01/02/04 + PA-05 Genie + PA-07/08 + PA-09/10 + PA-11/12) | 0 | 15 |
| **Total** | **55** | **1** | **30** |
