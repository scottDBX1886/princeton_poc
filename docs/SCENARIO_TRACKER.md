...
**Summary — prompt tested (the generate-from-prompt path):** **46 🟢 tested · 10 🟡 written · 30 — n/a.** Engineer 32 🟢 (E1 SE-04/05/06/07 · E3 SE-08 · E4 SE-10 · E5 SE-11–20 · E6 SE-03/21/22/23 · E7 SE-24/25/26/27 · E8 SE-28/29/30/31/32/33/35 · E9 SE-34) — pass complete except parked E2. Business Analyst 5 🟢 (BA-01/03/05/06/07). Data Scientist 8 🟢 (DS-01/02/04/05/06b/07/08/09), 1 🟡 (DS-03 — code-review-only, no classic compute), 1 — n/a (DS-06a, a reference guide with no object to generate). Platform Admin 1 🟢 (PA-05 Genie over the audit + lineage tables — space and grounding unchanged by the rebuild), 9 🟡 (PA-07…12 never tested; **PA-01/02/04 reverted from 🟢 to 🟡** because the 2026-08-22 rebuild replaced the role model in their shared prompt — the earlier pass tested the retired three-group text, not what is written now). — n/a means there is no object for a prompt to produce: SA-deployed jobs, UI walkthroughs, and reference guides.
...
| ID | Scenario | Covered by | Status | Prompt tested? |
|----|----------|-----------|--------|--------|
| PA-01 | User provisioning & role assignment | PA-A | ✅ built & verified in customer wksp (2026-08-23: `account users` is the one UC-grantable group; identity fns distinguish the two roles — as the role, session_user()='account users' and is_member('admins')=false) | 🟡 written — prompt REWRITTEN 2026-08-22 for session_user()/role-switch; the earlier 🟢 tested the old three-group prompt in the retired Azure wksp, so it does not attest to this text |
| PA-02 | Group-based access control | PA-A | ✅ built & verified in customer wksp (2026-08-23: grants keyed on a group, never a user; explicit grants isolated from inherited via inherited_from) | 🟡 written — prompt REWRITTEN 2026-08-22 (see PA-01); earlier 🟢 was against the retired three-group prompt |
...
| PA-04 | Source & target object-level permissions | PA-A | ✅ built & verified in customer wksp (2026-08-23: object-level denial shown against prod — UC privileges cascade, so dev's catalog-level ALL_PRIVILEGES makes in-dev withholding impossible; the over-permission audit surfaces that instead) | 🟡 written — prompt REWRITTEN 2026-08-22 (see PA-01); earlier 🟢 was against the retired three-group prompt |
...
| Admin (PA) | 1 (PA-05 Genie) | 9 (PA-01/02/04 re-test after the rebuild + PA-07…12) | 15 |
| **Total** | **46** | **10** | **30** |
