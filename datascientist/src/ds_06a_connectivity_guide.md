...
|---|---|---|
| **Server hostname** | Workspace URL, no `https://` | `adb-3438839487639471.11.azuredatabricks.net` |
...
The POC workspace is **Azure Databricks**, so hostnames end in
`.azuredatabricks.net` — not the `.cloud.databricks.com` form used in AWS documentation.
Get the exact host and HTTP path from **SQL Warehouse → Connection details**, which
renders ready-to-copy values for each client type.

**Never hardcode the token.** Every snippet below reads it from an environment variable:

```bash
export DATABRICKS_HOST="adb-3438839487639471.11.azuredatabricks.net"
...
Host            = adb-3438839487639471.11.azuredatabricks.net
Port            = 443
...
libname dbx jdbc
  ...
  url="jdbc:databricks://adb-3438839487639471.11.azuredatabricks.net:443/default;
